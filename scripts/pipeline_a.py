...
def process_demo_transcript(filepath):
    ...
    extraction_response_text = call_llm(build_extraction_prompt(transcript_text))

    try:
        account_memo = extract_json(extraction_response_text)
        account_memo["account_id"] = account_id
    ...
    agent_response_text = call_llm(build_agent_spec_prompt(account_memo))
